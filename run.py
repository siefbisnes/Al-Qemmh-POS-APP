...
class ServerController:
    ...
    def create_app(self):
        ...
        # lifetime - editing a .html file on disk then does nothing
        # until the app is fully closed and relaunched, which is easy
        # to mistake for a real bug while iterating on templates.
        # Setting ALQEMMA_DEV=1 in the environment before launching
        # turns that off so template edits show up on a plain page
        # refresh instead. Off by default - never enabled in a normal
        # install.
        if os.environ.get("ALQEMMA_DEV") == "1":
...
class AppAPI:
    ...
    def suppress_windows_key(self, enable):
        ...
        (task switch), and the OEM "launch app" row some keyboards
        ...
        if enable:
            ...
            OEM_LAUNCH_VKS = {
                0xB4, 0xB5, 0xB6, 0xB7,  # LAUNCH_MAIL, LAUNCH_MEDIA_SELECT, LAUNCH_APP1, LAUNCH_APP2
                0xA6, 0xA7, 0xA8, 0xA9, 0xAA, 0xAB, 0xAC,  # BROWSER_BACK..HOME
                0xB0, 0xB1, 0xB2, 0xB3,  # MEDIA_NEXT/PREV/STOP/PLAY_PAUSE
            }
